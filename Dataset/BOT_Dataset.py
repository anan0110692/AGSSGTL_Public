...
# %%
Dataset_name='BOT'
Num_of_Target_train_samples=10
def Source_data_generator(batch_size=1,num_workers=16):
   
    
    #############Load HSI for C2D######################################
    Datafile_path= os.path.join( os.path.dirname(os.path.abspath(__file__)),'Datafiles','BOT5_rfl_data.mat')
    HU_cube = sio.loadmat(Datafile_path)
    HU_cube = HU_cube['BOT5_data']
    HU_cube = HU_cube.astype(np.float32)
    ch_num = HU_cube.shape[-1]
    #
    ############ Load Train Mask################################
    Datafile_path= os.path.join( os.path.dirname(os.path.abspath(__file__)),'Datafiles','gt_5_bot.mat')
    TR_label = sio.loadmat(Datafile_path)
    TR_label = TR_label['cdata']
    ...
    Datafile_path= os.path.join( os.path.dirname(os.path.abspath(__file__)),'Datafiles','gt_7_bot.mat')
    sh_label = sio.loadmat(Datafile_path)
    sh_label = sh_label['cdata']
    sh_label = sh_label.astype(np.float32)
    N_sh_label = sh_label.copy()
    N_sh_label[N_sh_label==0]=-100
    # N_sh_label=mapping_mask(N_sh_label)
    
    N_sh_label = N_sh_label.astype(np.float32)
    original_unique_classes = np.unique( N_sh_label)

    
    # if Test_region == "shadow":
            
    TR_label[sh_label!=0]=-100
...
# %%
def Target_data_genrator(batch_size=1, num_workers=16,Num_of_target_train_samples=None,Labeling_spread=False):
    Datafile_path= os.path.join( os.path.dirname(os.path.abspath(__file__)),'Datafiles','BOT7_rfl_data.mat')
    HU_cube = sio.loadmat(Datafile_path)
    HU_cube = HU_cube['BOT7_data']
    HU_cube = HU_cube.astype(np.float32)
    ch_num = HU_cube.shape[-1]
    ############ Load Train Mask################################
   ...
   ###########################################################
    ...
    Datafile_path= os.path.join('Dataset/Datafiles','Fixed_Test_dataloader','Fixed_BOT_All_test_labels_idx.pkl')
    ...
    All_Val_labels_idx = np.array([])
    classes = np.unique(  N_sh_label.flatten())
    classes = classes[~(classes == 0)] # classes = classes[(classes == 13)]
    for i in classes:
        ...
        if Num_of_target_train_samples is not None or Labeling_spread==True:
              if Labeling_spread==True:
               Num_of_Samples=9
            # class_indices=class_indices[:Num_of_target_train_samples]
              class_train_unlabeled_idx,class_train_labeled_idx,_,_= train_test_split(class_indices, np.zeros((class_indices.shape[0],)), test_size= Num_of_target_train_samples, random_state=42)
       ...
       #######################################################################
        if  All_Val_labels_labeled_idx.size == 0:
          ...
          All_Val_labels_labeled_idx = np.concatenate(
                (All_Train_labels_labeled_idx, class_val_labeled_idx))
    ...
    if Labeling_spread==True:
        ...
        label_prop_model = LabelSpreading(kernel='knn', max_iter=10000,tol=0.000000001,gamma=0.1,alpha=0.8,n_neighbors=100)    
    ...
    # TR_label_com=Y_comp.reshape(TR_label.shape)
    TR_label_com=TR_label_complete[None,:,:]
    ...
    dataset_TR_com = dataf.TensorDataset(HU_cube, TR_label_com)
    train_com_loader = dataf.DataLoader(dataset_TR_com, batch_size=batch_size,num_workers=num_workers)
    ...
    # print(TR_label[TR_label!=-100].size())
    # print(Y_comp[Y_comp!=-100].size())
    # print(balanced_accuracy_score(Y_comp[Y_comp!=-100], Y_sparse[Y_comp!=-100]))
    # ConfusionMatrixDisplay.from_predictions(Y_comp[Y_comp!=-100], Y_sparse[Y_comp!=-100],normalize='true',cmap='Blues')
    # plt.show()
    ...
    # return train_unlabeled_loader, TEST_loader, train_com_loader,None, All_test_labels_idx

# %%
def Color_map_generator():
    Datafile_path= os.path.join( os.path.dirname(os.path.abspath(__file__)),'Datafiles','Botsowana_colorbar.mat')
    cmap = sio.loadmat(Datafile_path)
    cmap=cmap['cmap']
    UH_map = ListedColormap(cmap)
    return UH_map
...
class Source_Datamodule(L.LightningDataModule):
    ...
    def __init__(self, batch_size=3, num_workers=16  ):
        ...
        self.classes_labels_array=np.array( [' ',"Water","Primary Flood plain","Riparian","Firescar","Islan Interior","Woodlands","Savanna","Short Mopane","Exposed Soils",'Ignored'])
...
class Target_Datamodule(L.LightningDataModule):
    ...
    def __init__(self, batch_size=1, num_workers=16,Num_of_target_train_samples=None,Labeling_spread=False  ):
        super().__init__()
        with open('Dataset/Datafiles/Fixed_Test_dataloader/Fixed_BOT_test_dataloader.pkl', 'rb') as file:
           Target_test_Dataloader= dill.load(file)
        self.batch_size = batch_size
        self.num_workers = num_workers
        Target_data_generator_Output_tuple=Target_data_genrator(self.batch_size, self.num_workers,Num_of_target_train_samples=Num_of_target_train_samples,Labeling_spread=Labeling_spread)
...
class DA_Datamodule(L.LightningDataModule):
        ...
        def __init__(self, batch_size=1, num_workers=16, Source_Datamodule_arg=None,Number_of_Target_train_Samples=10 ,Labeled_spread=False):
            super().__init__()
            self.batch_size = batch_size
            self.num_workers = num_workers
            ...
            _Target_Datamodule=Target_Datamodule(self.batch_size, self.num_workers,Num_of_target_train_samples=Number_of_Target_train_Samples)
...
cfg={"LR_PRE": 10e-4,"EPOCH_PRE" :200,'EPOCH_DA':500,'LRT':10e-5,'LRD':10e-4,"number_of_target_labeled_sampels":10}
...
class Light_DAtrain(L.LightningModule):
  ...
  ##############################################################################################################
    ...
    def training_step(self, batch,batch_idx):
      ...
      # if  self.first:
      #     temp_mask=  batch[1][1]
      #     class_exist=torch.any(temp_mask==self.temp_class)
      #     if class_exist:
      #       self.first=False
      #       self.discriminator_attention_visuals_batch.append(batch[1][0])
      #       self.discriminator_attention_visuals_true_mask.append(batch[1][1])
      #-------------------------------------------------------------------------
