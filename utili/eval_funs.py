...
def eval_metrics(cnn,model_type,Test_loder,DA_Datamodule=None,GPU_NUM=None):
    ...
    # for batch in DA_Datamodule.Source_dataloader:
    #     xs,ys=batch
    
    # source_range=xs.max()-xs.min()
    ...
    for batch in Test_loder:
        ...
        l=torch.nn.CrossEntropyLoss()(out,(y).to(torch.long).cuda(GPU_NUM))
        loss_step.append(l.item())
    ...
    y=torch.cat(All_y,0)
    print(torch.tensor(loss_step).mean())
    ...
    # display_labels_with_map=labels_dip[original_classes[True_unique_classes_contigous.astype(np.int64)-1].astype(np.int64)]
    cm_display=ConfusionMatrixDisplay.from_predictions(y.cpu().detach().numpy().astype(np.int64),out.cpu().detach().numpy(),normalize='true',display_labels=display_labels_no_map,xticks_rotation=45,ax=ax,labels=True_unique_classes_contigous.astype(np.int64))
...
def inference_map(S_cnn=None,T_cnn=None,Target_test_dataloader=None,DA_Datamodule=None,name=None,GPU_NUM=None):
    ...
    if DA_Datamodule.Dataset_name=='C2Seg-AB':
    ...
    for i, batch in enumerate(Target_test_dataloader):
          ...
          if DA_Datamodule.Dataset_name=='C2Seg-AB':
            mapped_out_to_original_classes[torch.squeeze( GT)==-100]=0
          
          
         
          im=ax[0,i].imshow((mapped_out_to_original_classes),cmap=UH_map,norm=matplotlib.colors.Normalize(vmin=0,vmax=Values_all.size-1))
         
          ax[0,i].axis('off')
         
          selected_original_classes=original_classes[np.unique(out.detach().cpu().numpy().astype(np.int64))]#---
          ...
          fig.savefig('Inference_map_'+name+'.png',dpi=1600,bbox_inches='tight') 
